...
class Frdd(pl.LightningModule):
    ...
    def step(
        ...
        # self.print(
        #     f"x_hat min: {debiased_x_hat.detach().min().item()}",
        #     f"x_hat max: {debiased_x_hat.detach().max().item()}",
        #     f"x min: {batch.x.min().item()}",
        #     f"x max: {batch.x.max().item()}",
        #     f"denorm(x_hat) min: {self.denormalizer(debiased_x_hat.detach()).min().item()}",
        #     f"denorm(x_hat) max: {self.denormalizer(debiased_x_hat.detach()).max().item()}",
        #     f"denorm(x) min: {self.denormalizer(batch.x).min().item()}",

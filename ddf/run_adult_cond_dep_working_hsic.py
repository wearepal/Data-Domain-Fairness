...
def tf_median_pairwise_euclidean_distance(X):
    ...
    med_sqrt = get_median(sq_dist) #tf.contrib.distributions.percentile(sq_dist, 50)
    return med_sqrt
...
def dense_bn_relu(inp, units, deploy):
    ...
    #bn = tf.layers.batch_normalization(units)
    #if deploy:
    #    dp = tf.nn.dropout(units,keep_prob=1.0)
    #else:
    #    dp = tf.nn.dropout(units,keep_prob=0.8)
    return units #dp #tf.nn.relu(bn)
...
def quadratic_time_HSIC(data_first, data_second, sigma_first, sigma_second):
    ...
    #biased estimate
    hsic = tf.trace(tf.matmul(HKf,HLf,transpose_a=True))
    return hsic
...
def quadratic_time_MMD(data_first, data_second, data_third, data_fourth, sigma, 
    ...
    #X: data_first and data_third
    #S: data_second and data_fourth
    #Y: data_fifth and data_sixth
...
def compute_accuracy_multi_pvalue(Y, predictions, Xcontrol):
    ...
    for v in np.unique(Xcontrol):
        ...
        ii = ii + 1
    # pvalue = compute_p_rule(Xcontrol, predictions)
...
def compute_multi_fpr_fnr(Y,predictions,Xcontrol):
    #y2, p2, x2 = (Y,predictions,Xcontrol)
    #Y = np.array([y for (y, p, x) in zip(y2,p2,x2) if x[0] != x[1]]) #TODO: Xcontrol = [np.argmax(x) for x in Xcontrol] 
    #predictions = np.array([p for (y, p, x) in zip(y2,p2,x2) if x[0] != x[1]]) #TODO: delete this line
    #Xcontrol = [np.argmax(x) for (y, p, x) in zip(y2,p2,x2) if x[0] != x[1]] #TODO: delete this line
    Xcontrol = [np.argmax(x) for x in Xcontrol]
    ...
    tp = np.sum(np.logical_and(Y == +1.0, predictions == +1.0)) # something which is +ve AND is correctly classified as +ve
    tn = np.sum(np.logical_and(Y == 0.0, predictions == 0.0)) # something which is -ve AND is correctly classified as -ve
    ...
    ii = 0
    for v in np.unique(Xcontrol): #TODO: for v in set(Xcontrol)
        idx_ = Xcontrol == v
        fp = np.sum(np.logical_and(Y[idx_] == 0.0, predictions[idx_] == +1.0)) # something which is -ve but is misclassified as +ve
        fn = np.sum(np.logical_and(Y[idx_] == +1.0, predictions[idx_] == 0.0)) # something which is +ve but is misclassified as -ve
        tp = np.sum(np.logical_and(Y[idx_] == +1.0, predictions[idx_] == +1.0)) # something which is +ve AND is correctly classified as +ve
        tn = np.sum(np.logical_and(Y[idx_] == 0.0, predictions[idx_] == 0.0)) # something which is -ve AND is correctly classified as -ve
        ...
        fpr_fnr_tpr_sensitive[3,ii] = tnr ## TODO: I have added this line
        ii = ii + 1
...
def compute_fpr_fnr(Y,predictions,Xcontrol):
    ...
    fpr_fnr_tpr_sensitive = np.zeros((4,np.unique(Xcontrol).shape[0])) # This was 3, but I changed to 4 due to adding tnr
    ii = 0
    for v in np.unique(Xcontrol): # TODO: for v in set(Xcontrol):
        idx_ = Xcontrol == v 
        fp = np.sum(np.logical_and(Y[idx_] == 0.0, predictions[idx_] == +1.0)) # something which is -ve but is misclassified as +ve
...
class Model:
    ...
    def init_training(self, hsic_cost_weight, pred_cost_weight, dec_cost_weight, 
        ...
        x_map = rff_map.map(self.x) #TODO: removed rff here
        x_marginal_map = rff_map.map(self.x_marg) #TODO: removed rff here
        decoded_map = rff_map.map(self.decoded) #TODO: removed rff here
        decoded_marginal_map = rff_map.map(self.decoded_marginal) #TODO: removed rff here
        sens_map = self.s #rff_map_sens.map(self.s)  
        sens_marginal_map = self.s_marg #rff_map_sens.map(self.s_marg) 

        self.decoder_loss = tf.reduce_mean(tf.nn.l2_loss(self.x - self.decoded)) # TODO: self.decoded))    if learning x^
        #( tf.losses.absolute_difference(self.x , self.decoded) ) 
        ...
        if equalized_odds:
            ...
            #        tf.equal(self.s[:,1], 1)
            #    )
            #) # TODO: I think this is where we want to look at y = 0 - changed from 1
        ...
        self.pred_loss = (hsic_cost_weight * self.cycling_cost +
                          ...
                          pred_cost_weight * self.y_cost) # TODO: maybe add **2 to hsic cost
    ...
    def fit(self, train_data, train_data_marginal, valid_data, valid_data_marginal, SEED_NUM, logs_dir, verbose, tf_config,
...
def test(data_train,data_valid,data_test,features,logs_dir_f, SEED_NUM, model_config):
    ...
    cv = cross_validation.StratifiedKFold(y_train.flatten(), 10)
    #with Xtilde
    ...
    for iteration in all_iterations[-1:]:
        ...
        y_pred_valid, y_prob_valid, decoded_valid = model.predict(model,X_valid,logs_dir_f,tf_config,iteration, SEED_NUM)
        #decoded_valid = X_valid - decoded_valid #TODO: added as now learn x^, not x~
        y_pred_test, y_prob_test, decoded_test = model.predict(model,X_test,logs_dir_f,tf_config,iteration, SEED_NUM)
        #decoded_test = X_test - decoded_test #TODO: added as now learn x^, not x~
        ...
        for i, reg_const in enumerate(reg_array):
            ...
            #cv_scores[i]=cross_validation.cross_val_score(svm.LinearSVC(C=reg_const), decoded_train, y_train.flatten(), cv=cv)
        ...
        #clf = svm.LinearSVC(C=reg_best)
        clf.fit(decoded_train, y_train.flatten()) 

        ########on validation data
        #decoded_test = decoded_valid
        #y_test = y_valid
        #s_test = s_valid
        #######################
        predictions = clf.predict(decoded_test)
       #performance measurement
        acc,acc_sensitive = compute_accuracy_pvalue(y_test.flatten(), predictions, s_test.flatten()) # TODO: uncomment this line and delete line below
       #acc,acc_sensitive = compute_accuracy_multi_pvalue(y_test.flatten(), predictions, s_test)
        print ('SVM Accuracy: %.2f%%'%(acc*100.))
        print ("per sensitive value: %.2f, %.2f, (%.2f)"%(acc_sensitive[0]*100.,acc_sensitive[1]*100.,(acc_sensitive[0]-acc_sensitive[1])*100.))
        fpr,fnr,fpr_fnr_tpr_sensitive = compute_fpr_fnr(y_test.flatten(), predictions, s_test.flatten()) # TODO: Uncomment this line and delete line below
        #fpr,fnr,fpr_fnr_tpr_sensitive = compute_multi_fpr_fnr(y_test.flatten(), predictions, s_test)
        print ('SVM FPR and FNR: %.2f, %.2f'%(fpr*100.,fnr*100.))
        print ("TPR per sensitive value: %.2f, %.2f, (%.2f)"%(fpr_fnr_tpr_sensitive[2,0]*100.,fpr_fnr_tpr_sensitive[2,1]*100.,(fpr_fnr_tpr_sensitive[2,0]-fpr_fnr_tpr_sensitive[2,1])*100.))
        print ("FPR per sensitive value: %.2f, %.2f, (%.2f)"%(fpr_fnr_tpr_sensitive[0,0]*100.,fpr_fnr_tpr_sensitive[0,1]*100.,(fpr_fnr_tpr_sensitive[0,0]-fpr_fnr_tpr_sensitive[0,1])*100.))
        print ("FNR per sensitive value: %.2f, %.2f, (%.2f)"%(fpr_fnr_tpr_sensitive[1,0]*100.,fpr_fnr_tpr_sensitive[1,1]*100.,(fpr_fnr_tpr_sensitive[1,0]-fpr_fnr_tpr_sensitive[1,1])*100.))
        ...
        s_column_name = ["sensitive"]#'race_White', 'race_Amer-Indian-Eskimo']#'race_Asian-Pac-Islander']#, 'race_Black', 'race_White']
        y_column_name = ["label"]
    ...
    reg_best = reg_array[np.argmax(cv_mean)]
    clf = svm.LinearSVC(C=reg_best, dual=False, tol=1e-6, random_state=888)
    #clf = svm.LinearSVC(C=reg_best, tol=1e-8, random_state=0)
    ...
    print( "per sensitive value: %.2f, %.2f, (%.2f)"%(acc_sensitive[0]*100.,acc_sensitive[1]*100.,(acc_sensitive[0]-acc_sensitive[1])*100.))
    fpr,fnr,fpr_fnr_tpr_sensitive = compute_fpr_fnr(y_test.flatten(), predictions, s_test.flatten()) # TODO: uncomment this line and delete below
    #fpr,fnr,fpr_fnr_tpr_sensitive = compute_multi_fpr_fnr(y_test.flatten(), predictions, s_test)
    print('SVM FPR and FNR: %.2f, %.2f'%(fpr*100.,fnr*100.))
    print("TPR per sensitive value: %.2f, %.2f, (%.2f)"%(fpr_fnr_tpr_sensitive[2,0]*100.,fpr_fnr_tpr_sensitive[2,1]*100.,(fpr_fnr_tpr_sensitive[2,0]-fpr_fnr_tpr_sensitive[2,1])*100.))
    print("FPR per sensitive value: %.2f, %.2f, (%.2f)"%(fpr_fnr_tpr_sensitive[0,0]*100.,fpr_fnr_tpr_sensitive[0,1]*100.,(fpr_fnr_tpr_sensitive[0,0]-fpr_fnr_tpr_sensitive[0,1])*100.))
    ...
    for i, reg_const in enumerate(reg_array):
        cv_scores[i]=cross_validation.cross_val_score(svm.LinearSVC(C=reg_const, dual=False, tol=1e-6, random_state=888), decoded_train, y_train.flatten(), cv=cv)
        #cv_scores[i]=cross_validation.cross_val_score(svm.LinearSVC(C=reg_const, tol=1e-8, random_state=0), decoded_train, y_train.flatten(), cv=cv)
    print("CV_Scores", cv_scores)
    cv_mean = np.mean(cv_scores, axis=1)
    print("CV Mean", cv_mean)
    reg_best = reg_array[np.argmax(cv_mean)]
    ...
    acc,acc_sensitive = compute_accuracy_pvalue(y_test.flatten(), y_pred_test.flatten(), s_test.flatten()) # TODO: Uncomment this line and delete line below
    #acc,acc_sensitive = compute_accuracy_multi_pvalue(y_test.flatten(), y_pred_test.flatten(), s_test)
    print ('Encoder Accuracy: %.2f%%'%(acc*100.))
    print ("per sensitive value: %.2f, %.2f, (%.2f)"%(acc_sensitive[0]*100.,acc_sensitive[1]*100.,(acc_sensitive[0]-acc_sensitive[1])*100.))
    fpr,fnr,fpr_fnr_tpr_sensitive = compute_fpr_fnr(y_test.flatten(), y_pred_test.flatten(), s_test.flatten()) # TODO: Uncomment this line and delete line below
    #fpr,fnr,fpr_fnr_tpr_sensitive = compute_multi_fpr_fnr(y_test.flatten(), y_pred_test.flatten(), s_test)
...
if __name__ == '__main__':
    ...
    train(data_train,data_train_marginal,data_valid,data_valid_marginal,x_size,s_size,y_size,med_sq_dist,features,logs_dir_f, SEED_NUM)
